...
// ★ PORTED from AB-JChess: Chance Branch Reduction
// Reduce search depth for low-value piece reveals (pawns/advisors)
static inline Depth reveal_depth_reduction(PieceType pt, Depth baseDepth) {
    if (baseDepth < 3) return baseDepth;
    switch (pt) {
        case PAWN:    return baseDepth - 1;
        case ADVISOR: return baseDepth - 1;
            ...
            if (move != excludedMove && pos.legal(move))
            {
                ...
                if(pos.do_move_temp(move, st)){
                    ...
                    while (pos.getDark(darkSt, typecount, isDarkDepth))
                    {
                        ...
                        if (vTmp >= probCutBeta)
                            vTmp = -search<NonPV>(pos, ss + 1, -probCutBeta, -probCutBeta + 1, isDarkDepth ? 0 : reveal_depth_reduction(type_of(pos.piece_on(to_sq(move))), depth - 4), !cutNode);
...
dark_undo:
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    // qsearch had no time check and no stop check, and the jieqi dark-piece
    // recursion re-enters it at depth 0 without reducing depth, so a thread in
    // one of these tails could not be stopped by `go movetime` and bestmove
    // waited on the slowest helper. Same contract as search(): a value returned
    // after a stop is discarded by the caller.
    if (thisThread == Threads.main())
        static_cast<MainThread*>(thisThread)->check_time();
    if (Threads.stop.load(std::memory_order_relaxed))
        return VALUE_ZERO;
    ...
    while ((move = mp.next_move()) != MOVE_NONE)
    {
      ...
      if (pos.do_move(move, st, givesCheck)) {
          ...
          while (pos.getDark(darkSt, typecount, isDarkDepth))
          {
              vTmp = -qsearch<nodeType>(pos, ss + 1, -beta, -alpha, isDarkDepth ? 0 : reveal_depth_reduction(type_of(pos.piece_on(to_sq(move))), depth - 1));
              tryTypeTimes++;
              SC.append(pos.piece_on(to_sq(move)), vTmp, typecount);
              pos.setDark();
          }
